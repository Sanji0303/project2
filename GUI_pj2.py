...
# ==================== RECOMMENDATION ====================
    ...
    # ==================== PHẦN MỚI: TÌM KIẾM THEO NHU CẦU ====================
    ...
    # ==================== PHẦN CŨ: CHỌN BĐS ĐỂ ĐỀ XUẤT ====================
    st.subheader("🏠 Hoặc chọn bất động sản để xem đề xuất")
    
    # Hiển thị thống kê
    quan_stats = df['quan'].value_counts()
    col1, col2, col3 = st.columns(3)
    quans = list(quan_stats.index)
    for i, quan in enumerate(quans):
        if i == 0:
            with col1:
                st.metric(f"📍 {quan.upper()}", f"{quan_stats[quan]:,} BĐS")
        elif i == 1:
            with col2:
                st.metric(f"📍 {quan.upper()}", f"{quan_stats[quan]:,} BĐS")
        elif i == 2:
    ...
    if len(df_filtered) > 0:
        ...
        n_recommend = st.slider("Số lượng đề xuất:", 3, 10, 5, key="n_recommend")
        rec_type = st.radio("Loại đề xuất:", ["Hybrid", "Content-based"], key="rec_type")
        
        if st.button("🔍 Đề xuất", type="primary", key="recommend_btn"):
            # Lấy index gốc
            ...
            if len(sim_scores) == 0:
                ...
                st.divider()
                st.subheader(f"🏠 Kết quả đề xuất cho BĐS đã chọn:")
